...
def main(language, feature, n_epochs, remote, features, remote_update):
    if not remote:
    ...
    call('gcloud compute ssh --zone={zone} {vm} --command "nohup /home/shared/anaconda3/bin/python {remote_dir}/main.py {language} -r -e {epochs}{feature}{features} > run.out 2> run.err < /dev/null &" &'.format(
        zone=gcloud_zone,
        vm=gcloud_vm,
        remote_dir=gcloud_code_dir,
        language=language,
...
if __name__ == '__main__':
    ...
    main(args.language, args.feature, args.n_epochs, args.remote, args.features, args.update)
